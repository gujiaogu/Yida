...
public class BlueToothService {
	...
	public synchronized void connected(BluetoothSocket socket,
        ...
        LogWrapper.d("mConnectThread is null ? " + mConnectThread);
		if (mConnectThread != null) {
			mConnectThread.cancel();
			mConnectThread = null;
		}
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			int bytes;
			mHandler.obtainMessage(CONNECTION_READY, LOSE_CONNECT, -1)
					.sendToTarget();

			// Keep listening to the InputStream while connected
			while (!isCancel) {
				try {
//                    LogWrapper.d("connection waiting..., is cancel " + isCancel);
